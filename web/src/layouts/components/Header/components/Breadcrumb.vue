...
const breadcrumbList = computed(() => {
  ...
  // 菜单里没有匹配到当前路由时（例如首页看板），只展示首页面包屑，避免读取空数组报错
  if (!breadcrumbData.length || breadcrumbData[0].path !== HOME_URL) {
    breadcrumbData = [{ path: HOME_URL, meta: { icon: "HomeFilled", title: "看板" } }, ...breadcrumbData];
  }
